...
const App = () => {
  ...
  const forecasters = {
    simple_moving_avg: (timeSeries, horizon, window = 3) => {
      ...
      for (let i = 0; i < horizon; i++) {
        ...
        forecast.push({ point: avg, lower: avg - 1.96 * stdDev, upper: avg + 1.96 * stdDev });
    ...
    exponential_smoothing: (timeSeries, horizon, alpha = 0.3) => {
      ...
      for (let t = 1; t < timeSeries.length; t++) {
        residuals.push(Math.abs(timeSeries[t] - s));
        s = alpha * timeSeries[t] + (1 - alpha) * s;
      }
      const stdDev = Math.sqrt(residuals.reduce((a, b) => a + b * b, 0) / Math.max(residuals.length, 1));
      for (let i = 0; i < horizon; i++) {
        forecast.push({ point: s, lower: s - 1.96 * stdDev * (1 + i * 0.1), upper: s + 1.96 * stdDev * (1 + i * 0.1) });
      }
      return forecast;
    },
    ...
    linear_regression: (timeSeries, horizon) => {
      ...
      for (let i = 0; i < horizon; i++) {
        ...
        forecast.push({ point, lower: point - 1.96 * stdDev, upper: point + 1.96 * stdDev });
      }
      return forecast;
    },
    ...
    arima_simple: (timeSeries, horizon) => {
      ...
      for (let i = 0; i < horizon; i++) {
        ...
        forecast.push({ point: next, lower: next - 1.96 * stdDev * (1 + i * 0.05), upper: next + 1.96 * stdDev * (1 + i * 0.05) });
        lastValue = next;
      }
      return forecast;
    }
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (values[valueColumnIndex]) {
            parsedData.push({ timestamp: values[0] || `Period ${i}`, value: parseFloat(values[valueColumnIndex]) });
          }
        }
  ...
  const generateForecasts = async () => {
    ...
    try {
      ...
      for (let i = 0; i < forecastHorizon; i++) {
        const point = { period: data.length + i + 1, timestamp: `Forecast ${i + 1}` };
        selectedModels.forEach(modelId => {
      ...
      const chartData = data.map((d, i) => ({ period: i + 1, timestamp: d.timestamp, value: d.value }));
      selectedModels.forEach(modelId => {
        forecastsObj[modelId].forEach((val, i) => {
          if (!chartData[data.length + i]) {
            chartData[data.length + i] = { period: data.length + i + 1, timestamp: `Forecast ${i + 1}` };
          }
      ...
      setResults({ forecasts: combinedForecast, chartData: chartData, metrics: metricsObj, ensemble: !!ensembleForecast, modelCount: selectedModels.length });
    } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          <div className="lg:col-span-1 space-y-4 md:space-y-6">
            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
              <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2"><Upload className="w-5 h-5" /> Data Upload</h2>
            ...
            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
              <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2"><Settings className="w-5 h-5" /> Transformations</h2>
              <select value={transformation} onChange={(e) => setTransformation(e.target.value)} className="w-full bg-slate-700 text-white rounded px-3 py-2 border border-slate-600 text-sm">
            ...
            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
              ...
              <label className="flex items-center gap-3 mb-3">
                <input type="checkbox" checked={useEnsemble} onChange={(e) => setUseEnsemble(e.target.checked)} className="w-4 h-4 rounded" />
                <span className="text-white text-sm">Use Ensemble</span>
              </label>
              {useEnsemble && (
                <select value={ensembleMethod} onChange={(e) => setEnsembleMethod(e.target.value)} className="w-full bg-slate-700 text-white rounded px-3 py-2 border border-slate-600 text-sm">
              ...
              <div className="space-y-2">
                {models.map((model) => (
                  <label key={model.id} className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={selectedModels.includes(model.id)} onChange={() => handleModelToggle(model.id)} className="w-4 h-4" />
            ...
            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
              <label className="text-slate-300 text-sm mb-2 block">Periods: <span className="text-blue-400 font-bold">{forecastHorizon}</span></label>
              <input type="range" min="1" max="48" value={forecastHorizon} onChange={(e) => setForecastHorizon(parseInt(e.target.value))} className="w-full" />
            </div>
            <button onClick={generateForecasts} disabled={loading || data.length === 0 || selectedModels.length === 0} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-700 text-white font-semibold py-3 rounded-lg">
              {loading ? 'Generating...' : 'Generate'}
            </button>
            {results && (
              <button onClick={downloadResults} className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-lg flex items-center justify-center gap-2">
            ...
            {results ? (
              <>
                {Object.keys(results.metrics).length > 0 && (
                  <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                    <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2"><BarChart3 className="w-5 h-5" /> Accuracy Metrics</h2>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <ComposedChart data={results.chartData}>
                      ...
                      {selectedModels.map((modelId, idx) => (
                        <Line key={modelId} type="monotone" dataKey={modelId} stroke={colors[idx % colors.length]} name={models.find(m => m.id === modelId)?.name} strokeDasharray="5 5" dot={false} strokeWidth={2} />
                      ))}
                      {results.ensemble && (
